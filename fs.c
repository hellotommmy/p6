...
//---------------------------------------------------------------------------------------
//START of path_lookup

//return the required directory inode_number
 	...
 	if(s[0]=='/'){
  	...
  	while(type!=0){
		...
		{	
			//root
			break;
			/*printf("cd / should be here\n");*/
		}
		offset+=type;
		offset++;
		//printf("%s\n",t );
		dir = do_op(dir,t,mode);
		if(dir==ERROR)
			return ERROR;
		if(dir==-3){
			if(type!=0){
				return -3;//dir does not exist
			}
			else//last file not exist
				return ERROR;
		}
		if(dir==-4){
			int new_inode;
			new_inode = inode_alloc(DIRECTORY);
			if(new_inode==ERROR)
				return -4;
			insert_entry_into_dir(new_inode,work_dir,t);
			result = insert_entry_into_dir(new_inode,new_inode,".");
			if(result == ERROR){
				/*inode_free(work_dir);
				work_dir = temp_dir;*/
				return ERROR;
			}
			result = insert_entry_into_dir(work_dir,new_inode,"..");
			if(result == ERROR){
...
int do_op(int which_dir,char *t, int mode){
	if(mode==1){
		//want to find inode of name t in dir which_dir
		int res;
		res = find_file_inode_in_dir(which_dir,t);
		if(res==ERROR)
			return ERROR;
		return res;
	}
	...
	if(mode == 3){
		//find dir, used by open
		//work_dir will be changed into the path specified after this op
		int res;
		//cannot find file
		res = find_file_inode_in_dir(which_dir,t);
		if(res==ERROR)
...
//return -1 on error
//return 1 on a path(and truncate the path into a smaller one)
//if a path, t is used to store the file name
//END of path_lookup
//---------------------------------------------------------------------------------------
...
void ls_current(void){
 ...
 writeStr("Size\n");
  char block_buffer[BLOCK_SIZE];
  inode_t * inode_ptr;
  inode_ptr = inode_read(work_dir,block_buffer);
...
//  printf("wanna find %s,%d entries in directory %d\n",file_name, current_entries,directory_inode);
  ...
  {//TODO
	...
	if(i == how_many_blocks )
	{
	  ...
	  {
		 writeStr(dir_struct_ptr[j].name);
		 for(k = 0; k < MAX_FILE_NAME - strlen(dir_struct_ptr[j].name) + 1;k++)
		 	writeStr(" ");
		 /*print MAX_FILE_NAME - size of file name + 1 spaces*/
		 child_ino = inode_read(dir_struct_ptr[j].inode,child_blk);
		 /*writeStr(condition ? "D" : "F");*/
		 /*print 4 spaces*/
		 writeStr(child_ino->type == DIRECTORY ? "D" : "F");
		 writeStr("    ");
		 
		 itoa(dir_struct_ptr[j].inode,s);
		 writeStr(s);
		 writeStr("     ");
	/*	 writeInt(inode #);
		 print 5 spaces*/
...
//			ERROR_MSG(("out of range\n"))
	...
	if(open_inode_number == ERROR)
	{
		...
		int result;
		result = insert_entry_into_dir(open_inode_number,work_dir,t);
		if(result == ERROR)
		{
			inode_free(open_inode_number);
			return ERROR;
...
fs_mkdir( char *fileName) {
	...
	int temp_dir;
	temp_dir = work_dir;
	result = path_lookup(fileName,4,t);
/*	result = find_file_inode_in_dir(work_dir,fileName);*/
	if(result == ERROR || result==-4){//no such file
		work_dir = temp_dir;
		return ERROR;
	}
/*	int open_inode_number;
...
/*	result = insert_entry_into_dir(work_dir,work_dir,".");
	if(result == ERROR){
		inode_free(work_dir);
		work_dir = temp_dir;
		return ERROR;
...
fs_rmdir( char *fileName) {
	...
	char t[MAX_FILE_NAME];
	rm_inode_number = path_lookup(fileName,5,t);
	...
	if(inode->file_size < 2*sizeof(dir_t))
		return ERROR;
	parent = find_file_inode_in_dir(rm_inode_number,"..");
	if(recursive_rm(parent, rm_inode_number)!=ERROR){
		work_dir = temp_dir;
		return 0;
	}
	else{
...
int recursive_rm(int parent,int child){
	char child_buf[BLOCK_SIZE];
	inode_t * ino;
	ino = inode_read(child,child_buf);
	int result;
	// if(child is just a file)
	// 	delete file and return
	if(ino->type ==FILE_TYPE){
		result = remove_inode(parent,child);
		if(result==ERROR)
			return ERROR;
		inode_free(child);
		return 0;
	}
	// if(child has more than 2 entries)
	// {
	// 	for all the en that are not . or ..
	// 		recursive_rm(child,en) 
	// }
	if(ino->file_size > 2*sizeof(dir_t)){
		int n_ent;
		n_ent = ino->file_size/sizeof(dir_t);
		int how_many_blocks;
		int last_entry_last_block;
		how_many_blocks = n_ent / ENTRIES_PER_BLOCK;
		last_entry_last_block = n_ent % ENTRIES_PER_BLOCK;   
		...
		for(i = 0;i < how_many_blocks|| i==how_many_blocks &&last_entry_last_block!=0; i++)
		{//TODO
			temp = find_block(ino,i);
			//printf("current_entries:%d, how_many_blocks:%d, i:%d, temp:%d\n",current_entries,how_many_blocks, i,temp );
			block_read(temp,data_buffer);
			//block_read(ino->dir_blk_addr[i],data_buffer);
			dir_struct_ptr = (dir_t *)data_buffer;
			if(i == how_many_blocks )
			{
				//final block
				for(j = 0; j < last_entry_last_block; j++)
				{
					if(strcmp(".",dir_struct_ptr[j].name) == 0||strcmp("..",dir_struct_ptr[j].name) == 0)
					{
						continue;
					}
					if(recursive_rm(child,dir_struct_ptr[j].inode)==ERROR)
						return ERROR;
					//check if blocks needs to be
				}
			}
			else
			{
			...
			for(j=0;j<offset;j++){
				if(entry[j].inode == inode_number){
					if(j == offset - 1){
						...
						if((1 + (parent_inode->file_size -1 ) / BLOCK_SIZE) < parent_inode->n_blocks){
...
fs_cd( char *dirName) {
	...
	char t[MAX_FILE_NAME];
	dir_inode = path_lookup(dirName,2,t);
